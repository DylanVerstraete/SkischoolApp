...
export class UserService {
  private headers = new Headers({'Content-Type': 'application/x-www-form-urlencoded'});
    ...
    getCurrentUser(){
        ...
        return this.http.get("http://127.0.0.1:5000/api/users/" + currentUser.email, this.jwt()).map((response: Response) => response.json());
    }
    ...
    update(user: User) {
        return this.http.post('http://127.0.0.1:5000/api/users/edit/' + user.email, user, this.jwt()).map((response: Response) => response.json());
    }
    ...
    undoMember(user: User){
        return this.http.post('http://127.0.0.1:5000/api/delete/member' , user, this.jwt()).map((response: Response) => response.json());        
    }
